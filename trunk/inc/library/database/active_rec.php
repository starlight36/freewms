...
class db_active_record extends db_driver {
	...
	/**
	 * Processing public function for the four public functions above:
	 *
	 *	select_max()
	 *	select_min()
	 *	select_avg()
	...
	public function _max_min_avg_sum($select = '', $alias = '', $type = 'MAX') {
		...
		if ( ! in_array($type, array('MAX', 'MIN', 'AVG', 'SUM'))) {
			show_error('Invalid public function type: '.$type);
		}
	 ...
	 * this public function is here for backwards compatibility, as
	...
	/**
	 * The "set" public function.  Allows key/value pairs to be set for inserting or updating
	 *
	 * @access	public
	 * @param	mixed
	 * @param	string
	 ...
	 * Get
	 *
	 * Compiles the select statement based on the other public functions called
	 * and runs the query
	 *
	 * @access	public
	 ...
	 *
	 * Generates a query string based on which public functions were used.
	 * Should not be called directly.  The get() public function calls it.
	 *
	 * @access	private
	 * @return	string
	 */
	private function _compile_select($select_override = FALSE) {
		...
		if ($select_override !== FALSE) {
			...
			if (count($this->ar_select) == 0) {
				...
				// The reason we protect identifiers here rather then in the select() public function
				// is because until the user calls the from() public function we don't know if there are aliases
	 ...
	 * Merge Cache
	 *
	 * When called, this public function merges any cached AR arrays with
	 * locally called ones.
	 *
	 * @access	private
	 * @return	void
	...
	/**
	 * Resets the active record values.  Called by the get() public function
	 *
	 * @access	private
	 ...
	 *
	 * Called by the insert() update() and delete() public functions
	 *
	 * @access	private
	 * @return	void
	 */
