		...
		<div id="tabcontent">
			<div class="showsimplecon">
              ...
              <p><span class="left">排序: </span>
                <input type="text" class="topbar_order" name="topbar_order" value="<?php echo Form::set_value('topbar_order', $tinfo['topbar_order']);?>" />
				  <?php echo Form::get_error('topbar_order', '<span class="fielderrormsg">', '</span>');?>
                </p>
				<p><span class="left">链接类型: </span>
			    <label><input type="radio" name="topbar_type" value="0"<?php if(Form::set_value('topbar_type', $tinfo['topbar_type']) == '0'){echo ' checked="checked"';} ?> />&nbsp;外部链接</label>
				<label><input type="radio" name="topbar_type" value="0"<?php if(Form::set_value('topbar_type', $tinfo['topbar_type']) == '1'){echo ' checked="checked"';} ?> />&nbsp;分页部链接</label>
                <label><input type="radio" name="topbar_type" value="0"<?php if(Form::set_value('topbar_type', $tinfo['topbar_type']) == '2'){echo ' checked="checked"';} ?> />&nbsp;单页链接</label>
			     <?php echo Form::get_error('topbar_type', '<span class="fielderrormsg">', '</span>');?>
				</p>
              <p><span class="left">导航URL: </span>
			  ...
			  <p><span class="left">选着分类： </span>
					<select onchange="window.main.location.href='index.php?m=admin&a=content&do=save&cid='+this.value;" style="width:140px">
						<option value="main" id="df_main" selected="selected" disabled="disabled">选着目标</option>
						<?php echo $cate_select_tree; ?>
					</select>
              </p>
              <?php /**?><p><span class="left">选着单页： </span>
               		<select onchange="window.main.location.href='index.php?m=admin&a=content&do=save&cid='+this.value;" style="width:140px">
							<option value="main" id="df_main" selected="selected" disabled="disabled">选着单页</option>
							<?php echo $_select_tree; ?>
						</select>
              </p><?php */?>
              <p><span class="left">绑定ID: </span>
                <input type="text" class="text normaltext" name="topbar_bindid" value="<?php echo Form::set_value('topbar_bindid', $tinfo['topbar_bindid']);?>" />
				  <?php echo Form::get_error('topbar_bindid', '<span class="fielderrormsg">', '</span>');?>
                </p>
			    ...
			    <?php echo Form::get_error('topbar_target', '<span class="fielderrormsg">', '</span>');?>
              	...
              	<p><span class="left">是否显示/隐藏: </span>
